...
//
// This file defines the Antler configuration schema, and is generated by
// Antler when the config is loaded. Any modifications to it will be lost, so
// make all config changes in your own CUE files.

// When this file is generated, the package is changed to match the package of
// the other CUE files in the project's directory. This is done to avoid
// problems in CUE with poor error handling across packages. In the source tree,
// this must be the string "package antler" so it's properly replaced.
package antler
