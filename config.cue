...
//
// OutPathTemplate is a Go template (https://pkg.go.dev/text/template) used to
// generate paths for output files given the ID. Any path separators (e.g. '/')
// in the resulting string generated by the template will result in the
// creation of directories.
//
// DataFile sets the suffix of the gob output file used to save the raw result
// data. If empty, it will not be saved. In that case, the runtime overhead for
// saving the raw data is avoided (a minimal gain), but the test must always be
...
#Test: {
	...
	OutPathTemplate: string & !="" | *"{{range $v := .}}{{$v}}_{{end}}"
