...
// This Antler example config has tests of the DelTiC AQM, using various CCAs
// (Congestion Control Algorithms). One stream is run from the left to right
// endpoint. A third of the way through the test, the rate drops from rate0 to
// rate1, then returns to rate0 two-thirds through the test.
...
Run: {
	Serial: [
		...
		for c in [
				...
				"linear-b",
				"linear-c",
		] {{_qdisc: q, _bursty_udp: b, _cca: c} & _qdiscTest},
	]
}

// qdiscTest is the per-qdisc test
_qdiscTest: {
	// qdisc, bursty_udp and cca are parameters for each TestRun
	_qdisc:      string
	_bursty_udp: bool
	_cca:        string
