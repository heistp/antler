...
// Reader returns a ReadCloser for reading result data.
func (t *Test) Reader(name string) (rc io.ReadCloser, err error) {
	var p string
...
//
// If name is "-", a WriteCloser that writes to os.Stdout is returned, with a
// Close implementation that does nothing.
//
...
// file referred to by name already exists. If name is "-", overwrite is
// ignored.
//
...
// OutputPrefix is returned. The returned path is cleaned with filepath.Clean().
func (t *Test) OutputPath(name string) (path string, err error) {
	...
	path = filepath.Clean(b.String() + name)
	return
}
